...
<?php
	$real_name_checked = ($write['wr_9']) ? ' checked' : '';
	$real_name_txt = (isset($boset['nuse']) && $boset['nuse']) ? '익명글' : '실명글'; 
	$option = '<label class="control-label sp-label"><input type="checkbox" id="real_name" name="wr_9" value="1" '.$real_name_checked.'> '.$real_name_txt.'</label>';
	$option_hidden = '';
	if ($is_notice || $is_html || $is_secret || $is_mail) {
		if ($is_notice) {
			$option .= "\n".'<label class="control-label sp-label"><input type="checkbox" id="notice" name="notice" value="1" '.$notice_checked.'> 공지(실명)</label>';
		}
