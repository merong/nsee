...
for ($i=0; $i < $list_cnt; $i++) { 
	...
	if ($list[$i]['is_notice']) { // 공지사항
		$choice_icon = '*';
	} else if($list[$i]['as_choice_cnt'] >= $choice_limit) {
		$is_choice = true;
		$choice_icon = '<span class="tack-icon bg-orangered">완료</span>';
	} else {
		$choice_date = G5_SERVER_TIME - ($list[$i]['date'] + ($boset['end'] * 86400));
		if($list[$i]['wr_comment'] < $boset['limit_cmt'] && $choice_date > 0) {
			$choice_icon = '<span class="tack-icon bg-gray">보류</span>';
		} else {
			$choice_icon = '<span class="tack-icon bg-blue">진행</span>';
		}
	}

	if (!$is_choice && $list[$i]['icon_new']) {
		$choice_icon = '<span class="tack-icon bg-green">새글</span>';
	}
...
?>
	<li class="list-item<?php echo $li_css;?>">
		...
		<div class="wr-subject">
			...
			<?php if(!$list[$i]['is_notice']) { //공지가 아닐경우 ?>
				<div class="item-details text-muted font-12 visible-xs ellipsis">
					...
					<?php } ?>
					<?php if($list[$i]['as_choice'] > 0) { ?>
						<span><i class="fa fa-gift"></i> <?php echo number_format($list[$i]['as_choice']);?></span>
					<?php } ?>
					<span><i class="fa fa-commenting"></i> <?php echo ($list[$i]['as_choice_cnt'] > 0) ? $list[$i]['as_choice_cnt'] : '-';?> / <?php echo $choice_limit;?></span>
		...
		<div class="wr-date hidden-xs">
			<?php echo ($list[$i]['as_choice_cnt'] > 0) ? '<span class="red">'.$list[$i]['as_choice_cnt'].'</span>' : '-';?> / <?php echo $choice_limit;?>
		</div>
		<?php if($is_date) { ?>
			<div class="wr-date hidden-xs">
