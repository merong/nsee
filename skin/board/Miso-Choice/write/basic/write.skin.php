...
<div class="form-group">
	<label class="col-sm-2 control-label">채택 <?php echo AS_MP;?></label>
	<div class="col-sm-4">
		<div class="input-group input-group-sm">
			<input type="text" class="form-control input-sm" name="as_choice" id="as_choice" value="<?php echo ($write['as_choice']) ? $write['as_choice'] : (int)$boset['min_cp'];?>">
			<span class="input-group-addon" style="border-left:0; border-right:0;"><?php echo AS_MP;?></span>

			<input type="text" class="form-control input-sm" name="wr_1" id="wr_1" value="<?php echo ($write['wr_1']) ? $write['wr_1'] : 1;?>">
			<span class="input-group-addon">명</span>
		</div>
	</div>
	<?php if($boset['cpoint'] > 0 && (!$boset['fee'] || $boset['fee'] == "1")) { ?>
		<div class="col-sm-6">
			<label class="control-label sp-label text-muted">
				채택시 등록한 <?php echo AS_MP;?>의 <?php echo $boset['cpoint'];?>%가 수수료로 추가 차감됩니다.
			</label>
		</div>
	<?php } ?>
	<?php if($boset['min_cp'] > 0 || $boset['max_cp'] > 0) { ?>
		<div class="col-sm-10 col-sm-offset-2 text-muted" style="padding-top:4px;">
			채택 <?php echo AS_MP;?>는
			<?php if($boset['min_cp'] > 0) { ?>
				<?php echo number_format($boset['min_cp']);?> 이상
			<?php } ?>
			<?php if($boset['max_cp'] > 0) { ?>
				<?php echo number_format($boset['max_cp']);?> 이하
			<?php } ?>
			등록할 수 있습니다.	
		</div>
	<?php } ?>
...
function fwrite_submit(f) {
	...
	<?php if((int)$boset['min_cp'] > 0) { ?>
		if(cpoint < <?php echo $boset['min_cp'];?>) {
			alert("<?php echo $boset['min_cp'];?><?php echo AS_MP;?> 이상 등록할 수 있습니다.");
			f.as_choice.focus();
			return false;
		}
	<?php } ?>
	<?php if((int)$boset['max_cp'] > 0) { ?>
		if(cpoint > <?php echo $boset['max_cp'];?>) {
			alert("<?php echo $boset['max_cp'];?><?php echo AS_MP;?> 이하로만 등록할 수 있습니다.");
			f.as_choice.focus();
			return false;
		}
	<?php } ?>
