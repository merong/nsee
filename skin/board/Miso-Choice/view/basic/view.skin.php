...
$attach_list = '';
if ($view['as_choice'] > 0) {
	$choice_limit = (int)$view['wr_1']; // 여분필드 1
	$choice_limit = ($choice_limit > 0) ? $choice_limit : 1;
	$attach_list .= '<a class="list-group-item"><span class="black"><i class="fa fa-gift green"></i> 채택되신 <b class="blue">'.$choice_limit.'</b>분께는 <b class="red">'.number_format($view['as_choice']).'</b> '.AS_MP.'를 드립니다.</span></a>'.PHP_EOL;
}
...
<?php if($view['as_choice_cnt']) include_once($view_skin_path.'/choice.skin.php'); // 채택목록 ?>
