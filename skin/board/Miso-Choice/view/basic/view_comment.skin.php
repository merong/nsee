			...
			<div class="media" id="c_<?php echo $comment_id ?>"<?php echo ($cmt_depth) ? ' style="margin-left:'.$cmt_depth.'px;"' : ''; ?>>
				...
				<div class="media-body">
					...
					<div class="media-content">
						...
						<?php echo $comment ?>
						<div class="cmt-btn">
							<?php if($write['mb_id'] && $write['mb_id'] == $member['mb_id']) { //채택 ?>
								<?php if($list[$i]['mb_id'] && $list[$i]['mb_id'] != $write['mb_id']) { //글쓴이는 제외 ?>
									<div id="choice_<?php echo $comment_id;?>" class="cmt-choice pull-left">
										<?php if($list[$i]['as_choice_cnt']) { //채택댓글 ?>
											<i class="fa fa-check circle tiny bg-blue"></i>채택됨
										<?php } else { ?>
											<a href="javascript:;" onclick="apms_choice('<?php echo $bo_table;?>', '<?php echo $wr_id;?>', '<?php echo $comment_id;?>');">
												<i class="fa fa-check circle tiny bg-green"></i>채택하기
											</a>
										<?php } ?>
									</div>
								<?php } ?>
