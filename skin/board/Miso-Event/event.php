...
if(isset($ed) && $ed) {
	...
	if($own[0]['is_win'] && $own[0]['is_confirm']) {
		...
		$ev['end'] = '';
		if($ev['win'] == "1") { // 1명이면
			apms_event_winner($bo_table, $wr_id, $ev); // 당첨자 뽑기
		} else {
			...
			//추가 1명 더뽑기
			$ev['win'] = 1;
			apms_event_winner($bo_table, $wr_id, $ev, $ex_mb); // 당첨자 뽑기
		}
...
if($ev['entry_point'] > 0) {
	...
	if($row['cnt']) {
	    ...
	    if ($member['mb_point'] < $ev['entry_point']) {
		    alert("보유 ".AS_MP."(".number_format($member['mb_point']).")가 참여 ".AS_MP."(".number_format($ev['entry_point']).") 보다 부족합니다.");
		}
	}
} else {
	$is_entry = false;
...
if(!$ev['type'] && isset($point) && $point) {
	...
	if($boset['limit']) {
		if ($point > $member['mb_point']) {
			alert("보유 ".AS_MP."(".number_format($member['mb_point']).")를 초과해서 입찰할 수 없습니다.");
		}
	}
...
if($is_entry) {
	...
	if($ev['spt_id'] && $ev['spt_rate'] > 0) {
		$support_point = round($ev['entry_point'] * ($ev['spt_rate'] / 100));
	    insert_point($ev['spt_id'], $support_point, "{$board['bo_subject']} $wr_id 이벤트 협찬", $bo_table, $wr_id, "이벤트협찬", 0, 1);
	}
}

// 입찰처리
$point = (isset($point) && $point > 0) ? $point : 0;
