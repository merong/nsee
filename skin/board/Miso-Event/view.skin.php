...
// 이벤트 정보
$ev = apms_event_info($bo_table, $wr_id, $view);
$win = apms_event_winner($bo_table, $wr_id, $ev);
$own = apms_event_own($bo_table, $wr_id, $member['mb_id']);
$own_cnt = count($own);
$win_point = apms_event_point($ev, $bo_table, $wr_id);

$is_own = ($ev['type']) ? false : true;
$end_time = strtotime($ev['end_datetime']) - G5_SERVER_TIME;

// 스크립트 메시지
$script_msg = '';
if($own[0]['is_confirm']) {
	$script_msg = '이벤트 당첨처리를 해 주셔야 다운받을 수 있습니다.';
} else if($own[0]['is_win']) {
	; // 당첨일 때
} else {
	$script_msg = '이벤트 당첨자만 다운받을 수 있습니다.';
}
		...
		<div class="btn-group" role="group">
			...
			<?php if ($write_href) { ?>
				<a role="button" href="<?php echo $write_href ?>" class="btn btn-<?php echo $btn2;?> btn-sm"<?php echo $modal_target;?>>
					<i class="fa fa-upload"></i><span class="hidden-xs"> 등록</span>
				</a>
			<?php } ?>
		</div>
		<div class="clearfix"></div>
