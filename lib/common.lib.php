...
function set_cookie($cookie_name, $value, $expire, $path='/', $domain=G5_COOKIE_DOMAIN, $secure=false, $httponly=true)
{
    ...
    setcookie(md5($cookie_name), base64_encode($value), G5_SERVER_TIME + $expire, $path, $domain, $secure, $httponly);
}
...
function subject_sort_link($col, $query_string='', $flag='asc')
{
    ...
    $url = get_params_merge_url($qstr_array);
...
//UUID 코드 생성
function uuidgen() {
	return sprintf('%04x-%04x-%04x-%04x-%08x',mt_rand(0, 0xffff), mt_rand(0, 0xffff), mt_rand(0, 0xffff), mt_rand(0, 0xffff), mt_rand(0, 0xffffffff));
}
?>