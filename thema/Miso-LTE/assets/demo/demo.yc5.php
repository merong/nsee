...
$hmenu[$i]['new'] = 'old';
$hmenu[$i]['on'] = ($mon == $msp) ? true : false;
$hmenu[$i]['href'] = G5_SHOP_URL.'/list.php?ca_id=10&amp;mon='.$msp.'&amp;pv='.THEMA.'&amp;spm=list&amp;sps='.urlencode('Miso-LTE|1');
$hmenu[$i]['name'] = $hmenu[$i]['menu'] = '상품 스타일';
$hmenu[$i]['icon'] = '<i class="fa fa-cube"></i>';
$hmenu[$i]['is_sub'] = true;

$n = 0;
$shmenu[$n]['m'] = array('list', 'Miso-LTE|1', '상품목록', '', 0);
$shmenu[$n]['s'][] = array('list', 'Miso-LTE|1', '일반형', '', 0);
$shmenu[$n]['s'][] = array('list', 'Miso-LTE|2', '더보기형', '', 0);
$shmenu[$n]['s'][] = array('list', 'Miso-LTE|3', '무한스크롤형', '', 0);

$n++;
$shmenu[$n]['m'] = array('item', '', '상품설명', '', 0);

$n++;
$shmenu[$n]['m'] = array('type', 'Miso-LTE|1', '상품유형', '', 0);
$shmenu[$n]['s'][] = array('type', 'Miso-LTE|1', '일반형', '', 0);
$shmenu[$n]['s'][] = array('type', 'Miso-LTE|2', '더보기형', '', 0);
$shmenu[$n]['s'][] = array('type', 'Miso-LTE|3', '무한스크롤형', '', 0);

$n++;
$shmenu[$n]['m'] = array('search', 'Miso-LTE', '상품검색', '', 0);

$n++;
$shmenu[$n]['m'] = array('use', 'Miso-LTE', '상품후기', '', 0);

$n++;
$shmenu[$n]['m'] = array('qa', 'Miso-LTE', '상품문의', '', 0);

$n++;
$shmenu[$n]['m'] = array('event', 'Miso-LTE', '이벤트', '', 0);
if(USE_PARTNER) {
	$n++;
	$shmenu[$n]['m'] = array('myshop', 'Miso-LTE', '마이샵', '', 0);
}

for($j=0; $j < count($shmenu); $j++) {
	$hmenu[$i]['sub'][$j]['line'] = $shmenu[$j]['m'][3];
	$hmenu[$i]['sub'][$j]['sp'] = $shmenu[$j]['m'][4];
	$hmenu[$i]['sub'][$j]['on'] = ($hmenu[$i]['on'] && $spm == $shmenu[$j]['m'][0]) ? true : false;
	switch($shmenu[$j]['m'][0]) {
		case 'list'		: $demo_mode = 'list.php?ca_id=10&amp;spm='; break;
		case 'item'		: $demo_mode = 'item.php?it_id=399&amp;spm='; break;
		case 'type'		: $demo_mode = 'listtype.php?spm='; break;
		case 'search'	: $demo_mode = 'search.php?spm='; break;
		case 'myshop'	: $demo_mode = 'myshop.php?id=amina&amp;spm='; break;
		case 'event'	: $demo_mode = 'event.php?spm='; break;
		case 'use'		: $demo_mode = 'itemuselist.php?spm='; break;
		case 'qa'		: $demo_mode = 'itemqalist.php?spm='; break;
		default			: $demo_mode = 'list.php?ca_id=10&amp;spm='; break;
	}
	$hmenu[$i]['sub'][$j]['href'] = G5_SHOP_URL.'/'.$demo_mode.urlencode($shmenu[$j]['m'][0]).'&amp;sps='.urlencode($shmenu[$j]['m'][1]).'&amp;mon='.$msp.'&amp;pv='.THEMA;
	$hmenu[$i]['sub'][$j]['name'] = $hmenu[$i]['sub'][$j]['menu'] = $shmenu[$j]['m'][2];
	$hmenu[$i]['sub'][$j]['new'] = 'old';
	$hmenu[$i]['sub'][$j]['is_sub'] = count($shmenu[$j]['s']);
	// 서브가 있으면	
	if($hmenu[$i]['sub'][$j]['is_sub']) {
		for($k=0; $k < $hmenu[$i]['sub'][$j]['is_sub']; $k++) {
			switch($shmenu[$j]['s'][$k][0]) {
				case 'list'		: $demo_mode = 'list.php?ca_id=10&amp;spm='; break;
				case 'item'		: $demo_mode = 'item.php?it_id=399&amp;spm='; break;
				case 'type'		: $demo_mode = 'listtype.php?spm='; break;
				case 'search'	: $demo_mode = 'search.php?spm='; break;
			...
			$hmenu[$i]['sub'][$j]['sub'][$k]['href'] = G5_SHOP_URL.'/'.$demo_mode.urlencode($shmenu[$j]['s'][$k][0]).'&amp;sps='.urlencode($shmenu[$j]['s'][$k][1]).'&amp;mon='.$msp.'&amp;pv='.THEMA;
			$hmenu[$i]['sub'][$j]['sub'][$k]['name'] = $hmenu[$i]['sub'][$j]['sub'][$k]['menu'] = $shmenu[$j]['s'][$k][2];
			$hmenu[$i]['sub'][$j]['sub'][$k]['new'] = 'old';
			$hmenu[$i]['sub'][$j]['sub'][$k]['is_sub'] = 0;
			$hmenu[$i]['sub'][$j]['sub'][$k]['line'] = $shmenu[$j]['s'][$k][3];
			$hmenu[$i]['sub'][$j]['sub'][$k]['sp'] = $shmenu[$j]['s'][$k][4];
			$hmenu[$i]['sub'][$j]['sub'][$k]['on'] = ($hmenu[$i]['sub'][$j]['on'] && $sps == $shmenu[$j]['s'][$k][1]) ? true : false;
		}
	}
}
...
if($mon == 'color' || $mon == 'main') {
	...
	switch($pvm) {
		...
		default		: $at_set['mfile'] = 'main-shop-basic'; break; 
	}
	...
	if($pvm == 'c9') { //모바일
		...
		// 메인파일
		switch($pvs) {
			case '12'	: $at_set['mfile'] = 'main-shop-mobile-fulldark'; break; 
			default		: $at_set['mfile'] = 'main-shop-mobile-fullwhite'; break; 
		}
...
if($ev_id || $ca_id || $it_id || $pid == 'itype' || $pid == 'myshop') {
	...
	// 상품목록
	if($sps) {
		set_session('miso-lte-list', $sps);
	}

	list($ls, $lo) = explode("|", get_session('miso-lte-list'));

	$list_skin = ($ls) ? $ls : 'Miso-LTE';
