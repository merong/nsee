...
if($q || $stx) {
	...
	//등록태그 현황
	$row = sql_fetch(" select count(*) as cnt from {$g5['apms_tag']} where cnt > 0 and idx NOT IN ('#','@') ");
	$total_cnt = $row['cnt'];
	...
	if($sort == 'studio') {
		$idx = '';
		$result = sql_query(" select * from {$g5['apms_tag']} where cnt > 0 and idx ='@' order by type, idx, tag, cnt desc limit 0, 1000");
		$total_page = 1;
		for ($i=0; $row=sql_fetch_array($result); $i++) {
			$list[$i] = $row;
		...
		$idx = '';
		$result = sql_query(" select * from {$g5['apms_tag']} where cnt > 0 and idx NOT IN ('#','@') order by type, idx, tag, cnt desc limit $start_row, $trow ");
		for ($i=0; $row=sql_fetch_array($result); $i++) {
			$list[$i] = $row;
			$list[$i]['href'] = './tag.php?q='.urlencode($row['tag']);
			$list[$i]['is_sp'] = ($idx != $row['idx']) ? true : false;
	...
	$skin_file = $skin_path.'/tag_search.skin.php';
}
