...
$before_day = date("Y-m-d", strtotime("-7 day", time()));
$today = date("Y-m-d");

$sql_common = " from {$g5['board_new_table']} a, {$g5['board_table']} b, {$g5['group_table']} c where a.bn_datetime between '{$before_day} 00:00:00' AND '{$today} 23:59:59' and a.bo_table = b.bo_table and b.gr_id = c.gr_id and b.bo_use_search <> '0' and b.bo_use_search <= '{$member['mb_level']}' and c.as_show <> '0' ";
...
if ($view == "w") $sql_common .= " and a.wr_id = a.wr_parent "; else if ($view == "c") $sql_common .= " and a.wr_id <> a.wr_parent "; else
    $view = '';
...
$redis_cache_key = "new_cached:" . hash("md5", json_encode($sql));
if ($cache->exists($redis_cache_key)) {
    $row = $cache->unserialize($cache->get($redis_cache_key));
} else {
    //syslog(LOG_INFO, __FILE__." LINE : ".__LINE__." ".__FUNCTION__." new count sql : ".$sql."\n");
    $row = sql_fetch($sql);
    $cache->set($redis_cache_key, $cache->serialize($row), 1800);
}
...
$redis_cache_key = "new_cached:" . hash("md5", $sql);


if ($cache->exists($redis_cache_key)) {
    $list = $cache->unserialize($cache->get($redis_cache_key));
} else {
    //syslog(LOG_INFO, __FILE__." LINE : ".__LINE__." ".__FUNCTION__." ".$_SERVER['REMOTE_ADDR']."  new sql : ".$sql."\n");
    $result = sql_query($sql);
    for ($i = 0; $row = sql_fetch_array($result); $i++) {
    ...
    $cache->set($redis_cache_key, $cache->serialize($list), 120);
}
